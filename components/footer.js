  ...
  fetch(localCSVPath)
    ...
    .then(data => {
      ...
      rows.forEach(row => {
        ...
        // Dynamically build social icons only if URLs exist
        let socialIconsHTML = "";
        ...
        if (post.ig_url) {
          socialIconsHTML += `
            <a href="${post.ig_url}" aria-label="Instagram">
        ...
        if (post.yt_url) {
          socialIconsHTML += `
            <a href="${post.yt_url}" aria-label="YouTube">
        ...
        if (post.x_url) {
          socialIconsHTML += `
            <a href="${post.x_url}" aria-label="X">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
